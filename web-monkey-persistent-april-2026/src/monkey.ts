...
const clickRandomLocatorElement = async (locator: Locator) => {
    ...
    if (count > 0) {
        const element = locator.nth(randomIndex(count));
        ...
        await element.click({
            ...
            steps: 20,
            button: Math.random() < 0.05 ? 'right' : 'left',
        });
        return true;
    }
...
const randomClick = async (page: Page) => {
    // Consent dialogs like to hide in frames.
    const frames = page.frames();
    if (!frames.length) console.error('huh, no frames?');
    const frame = frames[randomIndex(frames.length)];

    const locator = frame.locator('a[href], button, [onclick], input[type="button"], input[type="submit"]');
    const count = await locator.count();
    ...
    if (count > 0) {
        ...
        if (Math.random() < 0.9) {
            ...
            if (await clickRandomLocatorElement(acceptButtons)) return true;
        }
...
const randomFormFill = async (page: Page) => {
    ...
    if (count > 0) {
        ...
        await element.fill(randomText, { timeout: 1000, force: true });
        return true;
    }
...
export const weightedRandomAction = async (page: Page) => {
    ...
    for (const action of actions) {
        if (rand < action.weight) {
            if (await action.func(page)) return;
        }
        rand -= action.weight;
    }
