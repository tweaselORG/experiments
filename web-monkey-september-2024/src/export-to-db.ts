...
if (!dataDir || !databasePath)
    throw new Error('You need to provide the the path for the HAR files, and path to the database as the arguments.');

const db = sqlite3(databasePath);
db.prepare(
    `create table if not exists "requests" (
    "id" integer,
    "dataset" text default 'web-monkey-september-2024',
    "initiator" text,
...
(async () => {
    for (const type of ['no-interaction', 'monkey']) {
        const harFiles = await globby(type + '/*.har', { cwd: dataDir, absolute: true });
        ...
        for (const harFile of harFiles) {
            ...
            if (!(await fse.pathExists(metaFile))) {
                console.error(`Meta file for ${harFile} does not exist.`);
                continue;
            }
